import { P as PUBLIC_DOMAIN } from "../../../chunks/public.js";
import { p as prisma } from "../../../chunks/prisma.js";
const determineChangeFrequency = (date) => {
  const diff = Date.now() - date.getTime();
  if (diff < 1e3 * 60 * 60 * 24) {
    return "hourly";
  } else if (diff < 1e3 * 60 * 60 * 24 * 7) {
    return "daily";
  } else if (diff < 1e3 * 60 * 60 * 24 * 30) {
    return "weekly";
  } else if (diff < 1e3 * 60 * 60 * 24 * 365) {
    return "monthly";
  } else {
    return "yearly";
  }
};
const generateUrls = (type, data) => {
  return data.map(
    (item) => `
        <url>
            <loc>${PUBLIC_DOMAIN}/${type}/${item.id}</loc>
            <lastmod>${item.updatedAt?.toISOString()}</lastmod>
            <changefreq>${determineChangeFrequency(item?.updatedAt)}</changefreq>
...
async function GET() {
  ...
  return new Response(
    ...
    >
      ...
      ${generateUrls(
      "post",
      posts.map((post) => ({ id: post.slug, updatedAt: post.updatedAt }))
    )}
      <url>
